...
datafor2016_new <- datafor2016_edit[-c(1312,1313,1319,102,928), ]
#datafor2016_edit <- datafor2016_edit[-c(434:458),]

no_hour <- datafor2016_new[datafor2016_new$Comment == "Date not working",]
no_hour <- no_hour %>%
  drop_na(site)
rows_no_hour <- rownames(no_hour)
datafor2016_new <- datafor2016_new[-as.numeric(rows_no_hour),]
...
data2016_final <- datafor2016_errors  %>%
  mutate(date_time_obs = paste(obsdate, sprintf("%.2f", round(as.numeric(Hour.Exact),2)))) %>%
  select(4:11,13:21,25:35,38)

nonfunc2016 <- data2016_final[data2016_final$Days == 0,]
rows <- rownames(nonfunc2016)
data2016_final <- data2016_final[-as.numeric(rows),]
