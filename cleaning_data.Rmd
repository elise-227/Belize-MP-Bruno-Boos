...
#first obs needed
data2015_add <- data2015_final %>%
  mutate(min.date = if_else(as.Date(obsdate) < as.Date(Date_Place), obsdate, Date_Place)) %>%
  group_by(site) %>%
  summarize(First_Obs = min(min.date))

data2015_final <- full_join(data2015_final, data2015_add, by = "site")

camdataprep_2015 <- data2015_final %>%
  select(site, First_Obs, Date_Place, Days, Last.Record, Northing.1, Easting.1)
...
#camop2015 <- cameraOperation(camopdataframe2017, stationCol = "site", setupCol = "Date_Place", retrievalCol = "Last.Record", occasionStartTime = 0 , dateFormat = "%Y-%m-%d", writecsv = FALSE)
