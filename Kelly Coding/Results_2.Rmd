...
#effects plot
CI <- read.csv("Critical_intv.csv")

CI$Species <- as.factor(CI$Species)
CI$Cov <- as.factor(CI$Cov)

#cross zero?
CI$pass <- ifelse(sign(CI$X2.5) < 0  & lag(sign(CI$X97.5)) >= 0, 0, 1) #double check these bc wrong in some places
...
#Distance to Roads
Dist_Roads <- CI %>% group_by(Species, Cov) %>% filter(Cov == "Roads")
...
#Biomass
Biomass <- CI %>% group_by(Species, Cov) %>% filter(Cov == "Biomass")
...
#Streams
Streams <- CI %>% group_by(Species, Cov) %>% filter(Cov == "Streams")
...
##Natural Disturbance
NatD <- CI %>% group_by(Species, Cov) %>% filter(Cov == "Nat_Disturb")
...
setwd("D:/")
library("gridExtra")
tiff("draft_effectsplot.tiff", units = "in", width = 9, height = 9, res = 300)
