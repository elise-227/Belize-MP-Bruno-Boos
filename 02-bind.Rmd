...
forest_loss <- read_excel("table_FL.xlsx")

#fix FL cov
forest_loss$Forest_Los <- round(forest_loss$Forest_Los)
forest_loss <- forest_loss[,-2]
forest_loss$FL <- ifelse(forest_loss$Forest_Los == 0, "No", "Yes")

#join to covs
biomass <- left_join(biomass, forest_loss, by = c("CameraID" = "Name"))
...
#save
save(site_covs, file = "site_covs.RData")
```