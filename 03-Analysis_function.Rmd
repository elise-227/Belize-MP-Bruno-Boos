...
```{r detections}
#loop to make det histories so that we can add to results
...
for (species in species_list){
    ...
    unmarkedlist[[paste0(species,"unmarkedFrame")]] <- unmarkedFrameOccu(y = detect_hist$detection_history, siteCovs = covs_all)
}
```
...
#setwd("C:/Users/Kelly/Documents/Belize-MP-Bruno-Boos/Kelly Coding")
#load("p_agouti.Rda")
...
#intercept
predlist <- list()
predlist <- c(agouti_anthro_env, btapir_anthro_env, cguan_anthro_env, GHDove_anthro_env, grayfox_anthro_env)
  #,greatcura_anthro_env, jaguar_anthro_env, ocellatedturkey_anthro_env, ocelot_anthro_env, Paca_anthro_env, puma_anthro_env, redbrockdeer_anthro_env, wnosedcoati_anthro_env, wtdeer_anthro_env)

names(predlist) <- c("Agouti", "Baird's tapir", "Crested Guan", "Gray-headed Dove", "Gray Fox") 
  #,"Great Curassow", "Jaguar", "Ocellated Turkey", "Ocelot", "Paca", "Puma", "Red Brocket Deer", "White-nosed Coati", "White-tailed Deer")
...
#intercept
predlist <- list()
predlist <- c(agouti_anthro_env, btapir_anthro_env, cguan_anthro_env, GHDove_anthro_env, grayfox_anthro_env, greatcura_anthro_env, jaguar_anthro_env) #, ocellatedturkey_anthro_env, ocelot_anthro_env, Paca_anthro_env, puma_anthro_env, redbrockdeer_anthro_env, wnosedcoati_anthro_env, wtdeer_anthro_env)

names(predlist) <- c("Agouti", "Baird's tapir", "Crested Guan", "Gray-headed Dove", "Gray Fox","Great Curassow", "Jaguar") #, "Ocellated Turkey", "Ocelot", "Paca", "Puma", "Red Brocket Deer", "White-nosed Coati", "White-tailed Deer")
...
for (model in species){
  fit_top <- predlist[[model]]
  df_pred <- data.frame(x = predict(fit_top, "det", newdata = NULL, transform = TRUE, re.form = NULL, level = 0.95))
  df_p[df_p$species == model,] <- df_pred %>% summarise(mean = mean(x.Predicted), se = sd(x.Predicted, na.rm = TRUE)/sqrt(n())) %>% as.data.frame()
}
# 
# save(df_int, file = "df_int.Rda")
# write.csv(df_int, file = "df_int.csv")
