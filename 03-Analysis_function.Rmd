...
list #15991 obs >40
species_list <- names(list)
species_list <- species_list[16:20]
...
nd <- data.frame(Logging = "Yes", NEAR_DIST_ROADS = mean(NEAR_DIST_ROADS), mean_bioma = mean(mean_bioma), mean_NDVI = mean(NDVI), Season = "Rainy", Nat_Dist = Y, site = NULL)
...
predict(
  ...
  newdata = NULL,
  transform = TRUE,
  re.form = NULL, #NULL to include random effects, NA to ignore
  level = 0.95)
#
